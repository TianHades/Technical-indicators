...
1. **技术指标解读**（面向初学者，逐项简要说明当前数值的含义。日线和周线要分开解读，注意两者的异同）:
   - RSI 日线/周线分别处于什么区域？
   - MACD 柱日线/周线的方向和变化趋势？
   - KDJ 日线/周线各处于什么位置？是否有金叉死叉信号？
   - 均线排列日线 vs 周线的差异意味着什么？（短空长多？共振？）
   - 日线和周线的K线形态分别有哪些值得注意的反转或持续信号？两者是否一致还是矛盾？
   - 量比说明什么？市场参与度如何？
   - 最近的支撑和阻力在哪？
   - 综合日线和周线，当前市场给出的整体信号是什么？是短期波动还是中期趋势变化？

2. **操作建议**（基于当前技术信号给出方向性指引）:
   - 如果用户想做趋势跟踪，当前信号支持吗？
   - 如果用户想做区间交易，适合吗？
   - 如果用户想做逆势反弹，条件是否成熟？
   - 什么情况下"什么都不做"是最好的选择？
